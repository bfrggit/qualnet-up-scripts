...
BANDWIDTH_WIRELESS_SUBNET_SIMU_NET = 48000000
BANDWIDTH_WIRED_SUBNET = 1000000000
BANDWIDTH_ROUTER_TO_SERVER = 1000000000
...
DYNAMIC_BANDWIDTH_NUM_CASES = 20
DYNAMIC_BANDWIDTH_MIN = 200 * 1000
DYNAMIC_BANDWIDTH_UNIFORM_HALF_RANGE = 2400 * 1000
DYNAMIC_SEED_RAND = 0
...
scenarioAppPartFileName = scenarioDirName + "/#{SCENARIO_NAME}.app.part"
scenarioConfigPartFileName = scenarioDirName + "/#{SCENARIO_NAME}.config.part"
dynamicBandwidthFileNamePattern = scenarioDirName + "/dynamic_%d.bandwidth"
dynamicScenarioRunScriptName = scenarioDirName + "/dynamic_run.sh"
dynamicSimulationScriptName = scenarioDirName + "/dynamic_simu.sh"

# Generate scenario configuration
puts "Writing to file: " + scenarioConfigPartFileName
scenarioConfigFileObj = File.open(scenarioConfigPartFileName, "w")
...
LINK-BANDWIDTH #{BANDWIDTH_WIRED_SUBNET}
...
[ N#{MASK}-%s ] PHY802.11-DATA-RATE #{BANDWIDTH_WIRELESS_SUBNET_SIMU_NET}
...
[ N#{MASK}-%s ] SUBNET-DATA-RATE #{BANDWIDTH_WIRED_SUBNET}
...
scenarioConfigFileObj.puts "
[%s] LINK-PROPAGATION-DELAY #{PROPOGATION_DELAY_ROUTER_TO_SERVER}MS
[%s] LINK-BANDWIDTH #{BANDWIDTH_ROUTER_TO_SERVER}" \
...
FileUtils.cp(scenarioConfigPartFileName, scenarioTestConfigFileName)

puts "Writing to file: " + scenarioTestConfigFileName
scenarioTestConfigFileObj = File.open(scenarioTestConfigFileName, "a")
scenarioTestConfigFileObj.puts "# Links Fixed Bandwidth"
for j in 0...listAP.size
	itemAP = listAP[j]
	scenarioTestConfigFileObj.puts "[ %s %s ] LINK-BANDWIDTH %d" \
		% [IP_LINKS_SWITCH % itemAP[-1], \
			IP_LINKS_SWITCH % convertItemIndexToHostId(j, offset=true), \
			itemAP[1] * 1000]
end
...
puts "Writing to file: " + scenarioConfigPartFileName
scenarioConfigFileObj = File.open(scenarioConfigPartFileName, "a")
...
FileUtils.cp(scenarioConfigPartFileName, scenarioConfigFileName)

puts "Writing to file: " + scenarioConfigFileName
scenarioConfigFileObj = File.open(scenarioConfigFileName, "a")
scenarioConfigFileObj.puts "# Links Fixed Bandwidth"
for j in 0...listAP.size
	itemAP = listAP[j]
	scenarioConfigFileObj.puts "[ %s %s ] LINK-BANDWIDTH %d" \
		% [IP_LINKS_SWITCH % itemAP[-1], \
...
# Generate dynamic bandwidth
def dynamicBandwidthUniform(bwStatic, bwMin, bwHalfRange)
	[bwStatic + rand * bwHalfRange * 2 - bwHalfRange, bwMin].max
end

srand DYNAMIC_SEED_RAND
for j in 0...DYNAMIC_BANDWIDTH_NUM_CASES
	dynamicBandwidthFileName = dynamicBandwidthFileNamePattern % (j + 1)
	puts "Writing to file: " + dynamicBandwidthFileName
	dynamicBandwidthFile = File.open(dynamicBandwidthFileName, "w")
	dynamicBandwidthFile.puts "# Links Dynamic Bandwidth"
	for j in 0...listAP.size
		itemAP = listAP[j]
		dynamicBandwidthFile.puts "[ %s %s ] LINK-BANDWIDTH %d" \
			% [IP_LINKS_SWITCH % itemAP[-1], \
				IP_LINKS_SWITCH % convertItemIndexToHostId(j, offset=true), \
				dynamicBandwidthUniform(itemAP[1] * 1000, \
					DYNAMIC_BANDWIDTH_MIN, \
					DYNAMIC_BANDWIDTH_UNIFORM_HALF_RANGE)]
	end
	dynamicBandwidthFile.puts
	dynamicBandwidthFile.close
end

# Generate application specification
lsDataFileNum = []

puts "Writing to file: " + scenarioAppPartFileName
scenarioAppFileObj = File.open(scenarioAppPartFileName, "w")
...
FileUtils.cp(scenarioAppPartFileName, scenarioTestAppFileName)
...
# Generate dynamic run script
puts "Writing to file: " + dynamicScenarioRunScriptName
dynamicScenarioRunScriptObj = File.open(dynamicScenarioRunScriptName, "w")
dynamicScenarioRunScriptObj.puts "#!/bin/bash
$QUALNET_HOME/bin/qualnet dynamic_#{SCENARIO_NAME}.config 2>error.log"
# scenarioRunScriptObj.puts
dynamicScenarioRunScriptObj.close
FileUtils.chmod "u+x", dynamicScenarioRunScriptName
...
cp #{SCENARIO_NAME}.app.part #{SCENARIO_NAME}.app
echo \"UP #{itemMDC[-2]} #{itemServer[-2]} MDC path $1\" >> up.app
./run.sh"
# simulationScriptObj.puts
simulationScriptObj.close
FileUtils.chmod "u+x", simulationScriptName

# Generate dynamic simulation file
puts "Writing to file: " + dynamicSimulationScriptName
dynamicSimulationScriptObj = File.open(dynamicSimulationScriptName, "w")
dynamicSimulationScriptObj.puts "#!/bin/bash
if [ \"$#\" -lt 2 ] || [ \"$#\" -gt 4 ] ; then
...
cp #{SCENARIO_NAME}.app.part #{SCENARIO_NAME}.app
cp #{SCENARIO_NAME}.config.part dynamic_#{SCENARIO_NAME}.config
cat $1 >> dynamic_#{SCENARIO_NAME}.config
echo \"UP #{itemMDC[-2]} #{itemServer[-2]} MDC path $2 $3 $4\" >> up.app
./dynamic_run.sh"
# simulationScriptObj.puts
dynamicSimulationScriptObj.close
FileUtils.chmod "u+x", dynamicSimulationScriptName
