...
serverOutputNameWildcard = "daemon_mdc*.out"
serverOutputPathReg = \
	/(\d+)\/case\_(\d+)\/case\/([A-Za-z\_\d]+)\/rand\_(\d+)\/daemon\_mdc\d+\.out$/
...
for valFile in valFilesList
	cmd = "#{RUBY} calc_length.rb #{valFile[-1]} #{valFile[0]}"
...
for j in 0...table.size
	...
	puts variableXsRev[j].to_s + "," + table[j].map {|x| "%.2f" % x}.join(",")
end
