...
# Parse output file
timeUp = [nil] * listDS.size
for line in linesNoEmpty
	result = outLine.match line[0]
	next if not result
	identDS = result[1].to_i
	timeDS = result[2].to_f
	if identDS < 1 or identDS > listDS.size
		STDERR.puts "Invalid data chunk identifier: #{identDS}"
	else
		if not timeUp[identDS - 1]
			timeUp[identDS - 1] = timeDS
		else # Should be avoided by simulator
			STDERR.puts "Duplicate data chunk received: #{identDS}"
			timeUp[identDS - 1] = [timeUp[identDS - 1], timeDS].min
		end
	end
end

for j in 0...timeUp.size
	timeUp[j] = 1.0 / 0.0 if not timeUp[j]
end

# Calculate weighted overall utility
def utilityF(delta)
...
puts "%.6f" % weightedAverageUtility
